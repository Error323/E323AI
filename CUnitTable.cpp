...
void CUnitTable::generateCategorizationFile(std::string& fileName) {
	...
	file << "# This file is autogenerated: DO NOT MODIFY!\n\n";
...
unitCategory CUnitTable::categorizeUnit(UnitType *ut) {
	...
	// TODO: the best way to detect if unit with "floater" and "waterline" tags set is a SUB 
	// unit is to get its model height and detect if its top can be seeing out of water;
	// very hard for now, imho
	if (ud->movedata) {
		if (ud->movedata->subMarine 
		|| (ud->movedata->moveType != MoveData::Ship_Move && ud->movedata->depth > 100.0f))
			cats |= SUB;
	}
	
	// FIXME: BA's Epoch ship has waterline = 15, and is not SUB-capable
	if ((cats&(SEA|SUB)).none()) {
		if (ud->canSubmerge 
		|| (ud->waterline > 10.0f && !ud->canhover)
		|| ((cats&STATIC).any() && !ud->floater && ud->minWaterDepth > 0.0f)) {
			cats |= SUB;
		}
